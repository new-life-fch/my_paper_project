...
> 🔍 **全盘复查完成（2026-06-28，commit 4c8be8a）**：逐项审查实验有效性。①**无数据泄露**（train/val/test query 互斥，交集=0）；②**对比公平**（judge 与探针同 query_ids+labels）；③**发现并修复一个方法学问题**——显著性原用非配对 bootstrap（各 reader 独立重采样，edge 方差被高估、CI 偏保守），已改配对 bootstrap（`boot_indices` 共享重采样），重跑结论完全不变（3B internal_edge +0.077 p=0.0008、8B +0.060 p=0.004、attn-final 4/4 p<1e-4），证明结论稳健；④**深度衰减 6/6 成立**（峰值层>输出层）；⑤**judge yes-bias 已查**（base 模型全输出 P(yes)>0.5、真实正例率仅 0.20，但 AUC 阈值无关故对比公平，且印证因果增益臂饱和）；⑥因果脚本注入位置/方向层/维度匹配正确，diff-of-means 已注明 probe-free。**结论：实验链干净，唯一真 bug（非配对 bootstrap）已修且不改变任何结论。**
>
