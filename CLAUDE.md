...
nnsight 框架参考:
`nnsight/`，该目录下有CLAUDE.md文件，引导你使用框架

知识库:
`PDF论文知识库/`，论文为PDF格式，内容较长，不推荐阅读

项目代码:
项目根目录下，所有代码编写和修改都在这里进行，临时脚本也写在此目录。

Git仓库:
https://github.com/new-life-fch/my_paper_project.git（凭证已在 credential store 中配置）
...
- **图增强/动态检索：** GFM-RAG (图基础模型检索器, arXiv: 2502.01113), Context-GuidedDynamic Retrieval (arXiv: 2504.19436)
...
1. **禁止胡编乱造**，所有文献信息需可溯源
2. 文献报告中的 CrAM 和 ADR 是外部补充搜索发现，非知识库论文
3. EvidITI 分析基于文件名+标题推断，未获取原文全文
4. 幻觉检测已从当前方案中移除，仅聚焦文档相关性探针
5. 核心优势论证点是语义同源 + 数据效率 + 零额外模型部署，不是计算量优势
...
- [x] 配置远程GPU服务器SSH连接
- [x] 在服务器上安装Python环境 + nnsight + transformers
...
| 文件 | 路径 | 说明 |
|------|------|------|
| 文献调研报告 | `RAG检索优化文献调研报告.md` | 17篇论文完整分析 + 数据集/模型详细调研 |
| 实现计划 | `实现计划-Attention-Probe-RAG.md` | 完整伪代码 + 实验设计 |
| 讨论总结 | `讨论总结-研究思路梳理.md` | 核心思路与优势论述 |
| nnsight 参考 | `nnsight/CLAUDE.md` | nnsight 框架使用指南 |
