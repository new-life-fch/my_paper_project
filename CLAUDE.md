...
**命门安全**：4/4 模型 best_resid(内部) > final-layer(输出)，"内部 > 输出"核心论点全部成立。LLaMA 系 internal_edge > train_edge（内部效应主导，8B 上达训练的 3.4 倍）。
...
- → 论点比原假设更强：**不是"对齐压制输出"，而是无论是否对齐，内部始终比输出知道得多**。对齐改善了输出端、缩小了"训练 gap"，却没消除"内部 gap"。（待 8B-Inst / Mistral-Inst 验证此 pattern 是否跨尺寸/架构复现）

🔄 队列进行中：8B-Instruct 下载中(~7.5G/16G)，之后 Mistral-Inst。系统盘稳定 6%，数据盘 39%。
