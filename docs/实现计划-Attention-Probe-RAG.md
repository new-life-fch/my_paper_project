...
> **⚠️ 校正说明（2026-06-27 整理时添加）**
>
> 本文档为 2026-05-31 编写的早期实现计划，后续编码（`src/`，2026-06-04）对部分细节做了演进。
> 整理项目时确认以下三处差异，**以实际代码（`src/`）为源真相**，本文档相应描述已过时，保留仅供参考对照：
>
> 1. **Prompt 模板**：本文档 Phase 2a 用 `"Question: {query}\n\nPassage: {passage}"`，
>    实际代码用 `"Q: {query}\nP: {passage}"`。**以代码为准。**
> 2. **数据规模/正负比**：本文档设想 500 queries × (5 正 + 20 负) ≈ 1:4；
>    实际代码 `src/data.py` 每 query 上限 5 passages（约 2 正 + 3 负）。因 MS MARCO v1.1
>    每 query 正样本数有限，实际产出仍接近 1:4 不均衡，由 `class_weight="balanced"` +
>    ROC-AUC 主指标处理。**规模目标（Phase 1=500、Phase 2=2000 queries）不变。**
> 3. **Stage 1 选头指标**：本文档 Phase 3 按验证集 **accuracy**（`clf.score`）选 top-k 头；
>    实际代码当前按验证集 **ROC-AUC** 选头。**决定：两种指标都保留以供对比实验**
>    （accuracy 选头方案为待补充实现，见 `WORK_STATUS.md` 待办）。
