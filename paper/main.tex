...
\emph{attn}: top-head probe. The best residual layer and the top heads are
selected \emph{on validation} (not on test), consistent with the significance
analysis. On every model the best internal reader exceeds both the judge and
the output-representation probe. Significance in
Tables~\ref{tab:sig}--\ref{tab:scale}.}
\label{tab:main}
