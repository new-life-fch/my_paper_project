...
description: 学术论文搜索、引用分析、开放获取 PDF 判定与结构化元数据提取专用 Skill。已适配 Windows + Edge 浏览器环境，PubMed 和 Semantic Scholar API Key 已配置（见 references/api-keys.md）。Use when the user asks to search/find papers, do literature review/survey/systematic review/PRISMA work, get citation counts, export BibTeX/RIS-style references, find papers by author, inspect PDF/open-access availability, or work with arXiv, Semantic Scholar, OpenAlex, Crossref, Unpaywall, PubMed, Google Scholar, ACM DL, IEEE Xplore, Papers with Code, CNKI, ScienceDirect, Wiley, Springer, ACS, MeSH, JEL, MSC, or ACM CCS.
metadata:
  version: "1.2.0"
  adapted_for: "Windows + Microsoft Edge + PowerShell"
  api_keys: "PubMed ✅ | Semantic Scholar ✅ | IEEE 🔲"
  cdp_proxy: "localhost:3456 → Edge (port 9222)"
---
...
```powershell
# PowerShell 环境检测（推荐，本机 Node.js 在 PowerShell 中可用）
node -e "const fs=require('fs'),net=require('net'),p=require('path'),la=process.env.LOCALAPPDATA;const f=p.join(la,'Microsoft/Edge/User Data/DevToolsActivePort');try{const lines=fs.readFileSync(f,'utf8').trim().split(/\r?\n/);const port=parseInt(lines[0]);const s=net.createConnection(port,'127.0.0.1');s.once('connect',()=>{console.log('Edge CDP: ok (port '+port+')');s.destroy()});s.once('error',()=>{console.log('Edge CDP: not connected')})}catch(e){console.log('Edge CDP: not connected')}"
```

- **Node.js 22+**：本机已安装 v22.16.0（非沙箱）。仅使用 API 平台时可不检查。
- **Edge 远程调试**：仅在访问 Google Scholar 或其他需要浏览器自动化的平台时必需。在 Edge 地址栏打开 `edge://inspect/#remote-debugging`，勾选 **Allow remote debugging for this browser instance**。已验证可用（端口 9222）。
- **curl**：PowerShell 原生支持 `Invoke-WebRequest`，也可使用 curl.exe。

arXiv、Semantic Scholar、PubMed、Papers with Code 等 API 平台无需 Edge 远程调试即可使用。

**API Key 状态**（详见 `references/api-keys.md`）：
- **PubMed**：✅ 已配置（10 req/s）
- **Semantic Scholar**：✅ 已配置（1 req/s，所有端点共享）
- **IEEE Xplore**：🔲 未配置（需机构订阅，无 Key 可用 WebFetch 替代）

**CDP Proxy**：本机已配置，端口 3456，连接 Edge。启动命令：
```powershell
node c:\Users\FENG\.trae-cn\skills\academic-search\scripts\cdp-proxy.mjs
```
验证：`Invoke-WebRequest -Uri 'http://127.0.0.1:3456/health' -UseBasicParsing`
...
已知 DOI 或 arXiv ID 时，直接用 Semantic Scholar 精确查询（API Key 已配置，见 `references/api-keys.md`）：
```powershell
# DOI 查询
Invoke-WebRequest -Uri "https://api.semanticscholar.org/graph/v1/paper/DOI:{doi}?fields=title,authors,year,abstract,citationCount,openAccessPdf" -Headers @{'x-api-key'='s2k-TTMZFtiNKgd00nxkXzlaOT07hdvcVeqQeKGAFUkA'} -UseBasicParsing

# arXiv ID 查询
Invoke-WebRequest -Uri "https://api.semanticscholar.org/graph/v1/paper/ARXIV:{arxiv_id}?fields=title,authors,year,abstract,citationCount,openAccessPdf" -Headers @{'x-api-key'='s2k-TTMZFtiNKgd00nxkXzlaOT07hdvcVeqQeKGAFUkA'} -UseBasicParsing
```
...
node scripts/oa-pdf-download.mjs `
  --input d:\solo-paper\academic-search\output\results.json `
  --manifest d:\solo-paper\academic-search\output\manifest.json

node scripts/oa-pdf-download.mjs `
  --input d:\solo-paper\academic-search\output\results.json `
  --manifest d:\solo-paper\academic-search\output\manifest.json `
  --download `
  --out-dir d:\solo-paper\academic-search\PDF
```
...
```powershell
# Semantic Scholar 作者搜索（API Key 已配置）
Invoke-WebRequest -Uri "https://api.semanticscholar.org/graph/v1/author/search?query={author_name}&fields=name,affiliations,paperCount,citationCount" -Headers @{'x-api-key'='s2k-TTMZFtiNKgd00nxkXzlaOT07hdvcVeqQeKGAFUkA'} -UseBasicParsing

# 获取作者全部论文（分页）
Invoke-WebRequest -Uri "https://api.semanticscholar.org/graph/v1/author/{author_id}/papers?fields=title,year,citationCount,externalIds&limit=100&offset=0" -Headers @{'x-api-key'='s2k-TTMZFtiNKgd00nxkXzlaOT07hdvcVeqQeKGAFUkA'} -UseBasicParsing
```
...
通过 CDP Proxy 直连用户日常 Edge 浏览器，天然携带登录态。
...
```powershell
# 启动 CDP Proxy（本机已配置，通常已在运行中）
node c:\Users\FENG\.trae-cn\skills\academic-search\scripts\cdp-proxy.mjs
```

CDP Proxy 默认监听 `127.0.0.1:3456`（可通过 `CDP_PROXY_PORT` 环境变量覆盖）。启动后保持运行，不要关闭终端。
...
$resp = Invoke-WebRequest -Uri "$PROXY/new?url=https://scholar.google.com" -UseBasicParsing
$TARGET = ($resp.Content | ConvertFrom-Json).targetId

# 执行 JS 提取数据
Invoke-WebRequest -Uri "$PROXY/eval?target=$TARGET" -Method POST -Body 'document.title' -UseBasicParsing

# 点击元素（CSS 选择器）
Invoke-WebRequest -Uri "$PROXY/click?target=$TARGET" -Method POST -Body 'button[type=submit]' -UseBasicParsing

# 完成后关闭 tab
Invoke-WebRequest -Uri "$PROXY/close?target=$TARGET" -UseBasicParsing
```
